...
def download(youtube_id,output,limit):
    comments_table = pd.DataFrame(columns=['Author', 'Comment', 'Predict'])
    try:
        ...
        with io.open(output, 'w', encoding='utf8') as fp:
            for comment in download_comments(youtube_id):
                ...
                comments_table = comments_table.append(
                    pd.Series([comment['author'], comment['text'], 0], index=comments_table.columns),
                    ignore_index=True)
